...
# #Inception 最初提出的版本主要思想是利用不同大小的卷积核实现不同尺度的感知。Inception Module基本组成结构有四个成分。1*1卷积，3*3卷积，5*5卷积，
# 3*3最大池化。特征图先被复制成4份并分别被传至接下来的4个部分，最后在合并层进行合并。一个额外的1*1/3*3/5*5卷积层再加上一个ReLU层进行压缩降维，减少参数量。
